...
Tpl.getHtml = function (layer) {//Вызывается как для основных так и для подслойв tpls frame.
	
	if (layer.data || layer.json || layer.tplsm || layer.tplroot) {
		var tpls = Template.make(layer.tpl);//С кэшем перепарсивания
		var tpl = Tpl.getTpl(layer);
		...
		layer.data = Tpl.getData(layer); //подменили строку data на объект data
